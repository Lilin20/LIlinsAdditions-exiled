...
using LilinsAdditions.Main.Items;
...
public class PMERHandler
{
    ...
    public void OnButtonInteractCoin(ButtonInteractedEventArgs ev)
    {
    ...
    public void OnButtonInteract(ButtonInteractedEventArgs ev)
    {
    ...
    public void OnButtonInteractGobblegum(ButtonInteractedEventArgs ev)
    {
        ...
        var currentUsage = GetCurrentUsage(ev.Schematic);
        var newUsage = currentUsage + 1;
        VendingMachineUsage[ev.Schematic] = newUsage;

        Log.Debug(
            $"[Gobblegum] Interaction from '{ev.Schematic.name}' by {ev.Player.Nickname} (Usage: {newUsage}/{config.VendingMachineUsageLimit})");

        GiveRandomGobblegum(ev.Player);

        if (ShouldDestroyVendingMachine(newUsage, config.VendingMachineUsageLimit)) DestroyVendingMachine(ev.Schematic);
    }

    private static int GetCurrentUsage(SchematicObject schematic)
    {
        return VendingMachineUsage.TryGetValue(schematic, out var usage) ? usage : 0;
    }
    ...
    private static void DisableVendingMachineLights(SchematicObject schematic)
    {
        foreach (var block in schematic.AttachedBlocks)
            if (block.name.Contains("Light") || block.name.Contains("Lamp"))
            {
                ...
                if (lightSource != null) lightSource.NetworkLightIntensity = 0f;
    ...
    private static void GiveRandomGobblegum(Player player)
    {
        ...
        var randomItem = _cachedBuyableGobblegums[Random.Range(0, _cachedBuyableGobblegums.Count)];
