...
public class HackGun : CustomWeapon
{
    ...
    public override string Description { get; set; } =
        "A tool gun which locks doors when shot. Also causes a blackout on generators.";
    ...
    private static void ToggleDoorLock(DoorVariant doorVariant, Player shooter)
    {
        ...
        if (door.IsLocked)
        {
            ...
            {
                Log.Debug(
                    $"[HackGun] {shooter.Nickname} tried to unlock gate {door.Type} but gates cannot be opened by HackGun");
                return;
            }
