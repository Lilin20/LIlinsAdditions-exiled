...
public class Sniper : CustomWeapon
{
    ...
    protected override void SubscribeEvents()
    {
        ...
        Item.ChangingAttachments += OnChangingAttachments;
    ...
    {
        if (Check(ev.Player.CurrentItem)) ev.IsAllowed = false;
    }

    private void OnActivatingWorkstation(ActivatingWorkstationEventArgs ev)
    {
        if (Check(ev.Player)) ev.IsAllowed = false;
    }
    ...
    {
        if (!FriendlyFire && ev.Target.Role.Team == ev.Player.Role.Team)
            return;
    ...
    private void ApplyScpDamage(Player target)
    {
        target.Hurt(ScpDamage);
    }

    private void KillTarget(Player target)
    {
